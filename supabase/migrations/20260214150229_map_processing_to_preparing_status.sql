/*
  # تحويل حالة processing إلى preparing في قائمة السائقين
  
  1. المشكلة
    - الطلبات بحالة 'processing' في جدول orders يتم إضافتها بحالة 'pending' في driver_waiting_list
    - لكن 'processing' تعني أن البائع قَبِل الطلب وبدأ تحضيره
    - يجب أن تظهر للسائقين كطلبات "قيد التحضير"
  
  2. الحل
    - تحديث الدالة لتحول حالة 'processing' إلى 'preparing' في قائمة السائقين
...
BEGIN
  ...
  IF NEW.status IN ('processing', 'preparing', 'accepted', 'waiting-for-driver') 
    ...
    -- كل من processing و preparing تعني "قيد التحضير"
    IF NEW.status IN ('processing', 'preparing') THEN
...
-- تحديث جميع الطلبات الموجودة بحالة processing أو preparing
DO $$
DECLARE
  v_order RECORD;
  v_count INTEGER := 0;
BEGIN
  RAISE NOTICE '🔄 بدء تحديث الطلبات بحالة processing و preparing...';
  ...
  FOR v_order IN 
    ...
    WHERE o.status IN ('processing', 'preparing') AND dwl.status != 'preparing'
  LOOP
    UPDATE driver_waiting_list
    SET 
      status = 'preparing',
    ...
    v_count := v_count + 1;
    RAISE NOTICE '✅ تم تحديث الطلب % من حالة %s إلى preparing', v_order.order_number, v_order.status;
  END LOOP;
  
  IF v_count > 0 THEN
    RAISE NOTICE '✅ تم تحديث % طلب', v_count;
  ELSE
...
'يضيف ويحدث الطلبات في قائمة انتظار السائقين. الطلبات بحالة processing أو preparing تظهر كـ "قيد التحضير" للسائقين';
